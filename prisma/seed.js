...
async function main() {
    ...
    const posts = await Promise.all([
        ...
        prisma.post.create({
            data: {
                ...
                thumbnail: 'https://images.unsplash.com/photo-1555066931-bf19f8fd8865',
                published: true,
                authorId: users[0].id,
    ...
    await Promise.all([
        ...
        prisma.comment.create({
            data: {
                content: 'Next.js 14 thực sự là một bước tiến lớn! Server Actions giúp code của tôi gọn gàng hơn nhiều.',
                postId: posts[0].id,
                authorId: users[1].id
            }
        }),
        prisma.comment.create({
            data: {
                content: 'RSC là tương lai của React, nhưng vẫn cần thời gian để cộng đồng thích nghi với cách tiếp cận mới này.',
                postId: posts[1].id,
                authorId: users[2].id
            }
        }),
        ...
        prisma.comment.create({
            data: {
                content: 'Tôi đã thử cả Flutter và React Native, và phải nói Flutter có hiệu suất tốt hơn đáng kể.',
                postId: posts[2].id,
                authorId: users[3].id
            }
        }),
        ...
        prisma.comment.create({
            data: {
                content: 'Docker và Kubernetes đã thay đổi hoàn toàn cách chúng ta triển khai ứng dụng. Bài viết rất hữu ích!',
                postId: posts[3].id,
                authorId: users[4].id
            }
        }),
        ...
        prisma.comment.create({
            data: {
                content: 'ChatGPT đã giúp tôi tiết kiệm rất nhiều thời gian trong việc viết code và debug.',
                postId: posts[4].id,
                authorId: users[0].id
            }
        }),
        ...
        prisma.comment.create({
            data: {
                content: 'Rust có learning curve khá dốc, nhưng một khi đã quen, bạn sẽ thấy nó rất mạnh mẽ.',
                postId: posts[5].id,
                authorId: users[1].id
            }
        }),
        ...
        prisma.comment.create({
            data: {
                content: 'Microservices không phải là giải pháp cho mọi vấn đề. Đôi khi monolith vẫn là lựa chọn tốt hơn.',
                postId: posts[6].id,
                authorId: users[2].id
            }
        })
